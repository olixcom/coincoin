...
#if defined(NDEBUG)
# error "Coincoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x455e03a317c1178e89753a2b4f46750049d631e1b1d9d767af89df018fc2c56c");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Coincoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Coincoin Signed Message:\n";
...
{
    // Coincoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 100000); // Coincoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1.0 * 24 * 60 * 60; // Coincoin: 1 days
static const int64 nTargetSpacing = 1.0 * 60; // Coincoin: 1 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Coincoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Coincoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xe026ea471c952f6bf62ee67350dd183f40a6e2ddb119bdfc9b17ccd694cf947d");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Test du coincoin !!!";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 500000 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04498f336a2e9392ed25248f2be2c7289b47dd85feb029acc24a72be16dfe8eddd968d9db0d0ef6e5b7f53c73b3b9823987017b33a6d369b033e17211e2347780c") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1518475362;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085359756;

        if (fTestNet)
        {
            block.nTime    = 1518475343;
            block.nNonce   = 386370006;
        }
	...
	if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xe72f9cb588622ba6621951b0e71fe077b53196fd0f88ae8ddbee94de391de019"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc6, 0xb8, 0xd7 }; // Coincoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CoincoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CoincoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static CoincoinMiner(CWallet *pwallet)
{
    printf("CoincoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("coincoin-miner");
    ...
    try { loop {
        ...
        printf("Running CoincoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("CoincoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CoincoinMiner, pwallet));
}
